...
DynamicAdapt.prototype.init = function () {
  ...
  for (let i = 0; i < this.mediaQueries.length; i++) {
    ...
    const mediaSplit = String.prototype.split.call(
      media,
      ","
    );
...
DynamicAdapt.prototype.mediaHandler = function (
  ...
  if (matchMedia.matches) {
    ...
    for (let i = оbjects.length - 1; i >= 0; i--) {
      const оbject = оbjects[i];
      if (
        оbject.element.classList.contains(this.daClassname)
      ) {
        this.moveBack(
          оbject.parent,
          оbject.element,
...
// Функция перемещения
DynamicAdapt.prototype.moveTo = function (
  place,
  element,
  destination
) {
  ...
  if (
    place === "last" ||
    place >= destination.children.length
  ) {
...
// Функция возврата
DynamicAdapt.prototype.moveBack = function (
  parent,
  element,
  index
) {
...
DynamicAdapt.prototype.indexInParent = function (
  parent,
  element
) {
