...
export function formFieldsInit(
  ...
  textAreaRef.addEventListener("input", (e) => {
    if (e.target.value.length > 0) {
      const labelTextareaRef = document.querySelector(
        ".form__label.label-textarea"
      );
      labelTextareaRef.classList.add("hidden");
    }
  });
...
export let formValidate = {
  getErrors(form) {
    let error = 0;
    let formRequiredItems = form.querySelectorAll(
      "*[data-required]"
    );
  ...
  validateInput(formRequiredItem) {
    ...
    if (formRequiredItem.dataset.required === "email") {
      formRequiredItem.value =
        formRequiredItem.value.replace(" ", "");
      ...
      formRequiredItem.dataset.required === "number"
    ) {
      formRequiredItem.value =
        formRequiredItem.value.replace(" ", "");
  ...
  addError(formRequiredItem) {
    ...
    formRequiredItem.parentElement.classList.add(
      "_form-error"
    );
    let inputError =
  ...
  removeError(formRequiredItem) {
    ...
    formRequiredItem.parentElement.classList.remove(
      "_form-error"
    );
    if (
      formRequiredItem.parentElement.querySelector(
...
export function formQuantity() {
  document.addEventListener("click", function (e) {
    ...
    if (
      ...
      targetElement
        .closest("[data-quantity]")
        .querySelector("[data-quantity-value]").value =
        value;
    }
  });
}
...
export function formRating() {
  ...
  function initRatings() {
    ...
    function initRatingVars(rating) {
      ratingActive = rating.querySelector(
        ".rating__active"
      );
    ...
    function setRating(rating) {
      const ratingItems =
        rating.querySelectorAll(".rating__item");
      for (
        let index = 0;
        index < ratingItems.length;
        index++
      ) {
        const ratingItem = ratingItems[index];
        ratingItem.addEventListener(
          "mouseenter",
          function (e) {
